...
* R21.7. An inert-mechanism lint (`tests/unit/test_inert.py`). Every `self.X = ...` in the package is checked against every `.X` read in the package, its tests and its tools; a name written and never read is reported. It found five real cases on its first run, two carrying comments claiming they fed metrics that were never built. Its reach is narrow and stated in the module docstring: of the six inert mechanisms found by hand across seven reviews it would have caught one, because reads are matched by bare attribute name with no type information and module constants, dataclass fields, unused parameters and uncalled methods are outside it entirely. Exemptions live in an `ALLOWED` map and must state a reason.
