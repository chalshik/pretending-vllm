...
| `--enable-expert-parallel` + replicas | the replicas must step together, so an idle one runs a dummy pass rather than idling — one request on four EP replicas costs three dummy steps per real one, reported per replica |
...
| a state-space hybrid (Nemotron-H style) | Mamba layers hold a fixed recurrent state per request instead of KV — constant in context where KV is linear, equal at ~6,200 tokens — and the state forces the attention block size from 16 tokens to 1040, which moves block counts and every block hash |
